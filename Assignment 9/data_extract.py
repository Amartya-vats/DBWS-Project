...
class access_log:
    def __init__(self, ip=None, timestamp=None, file_visited=None, browser = None):
        self.ip = ip
    ...
    with open("all_logs", 'r') as file:
        for line in file:
            ...
            for match in browser:
                log.browser = match.group(0)[3:-1]
            if log.ip != None and log.file != None and log.timestamp != None and log.browser != None:
                logs.append(log)
    logs.sort(key= lambda x : x.numeric_timestamp())
    with open("./access_logs_filtered", 'w') as file:
        file.write("IP, Timestamp, File Accessed, Browser User\n")
        for log in logs:
            file.write(f"{log.ip},{log.timestamp},{log.file},{log.browser}\n")
...
def error_log_parser():
    logs = []
    with open("all_error_logs", 'r') as file:
        for line in file:
            log = error_log()
    ...
    write_file.write('Whom, When, Where, What')
    for log in logs:
        write_file.write(f"{log.whom},{log.when},{log.where},{log.what}\n")
    write_file.close()
